...
void	test_ft_write(char *str)
{
	...
	i = 0;
	fd = open("./test_files/test_write.txt", O_RDWR | O_CREAT, 0644);
	if (fd == -1) {
		perror("open");
		return;
	}
...
int		check_base(char *base)
{
	...
	{
		j = i + 1;
		if (base[i] == '-' || base[i] == '+' || (unsigned char)base[i] < 32 || (unsigned char)base[i] > 127 || base[i] == ' ')
			return (0);
		while (base[j])
		{
			if (base[i] == base[j])
...
int cmp(void *a, void *b) {
	return strcmp((char *)a, (char *)b); // Cast void* to char* before using strcmp
}
...
void	test_ft_list_remove_if(int size, char *str)
{
	...
	ft_list_remove_if(&list, str, strcmp, del);
	i = 0;
	new = list;
...
int		main(int ac, char **av)
{
	...
	else if (ac == 4)
	{
		...
		else if ((strcmp("ft_atoi_base", av[1]) == 0) || (strcmp("atoi_base", av[1]) == 0))
			test_ft_atoi_base(av[2], av[3]);
		else if ((strcmp("ft_list_remove_if", av[1]) == 0) || (strcmp("list_remove_if", av[1]) == 0))
